...
In order to understand parallelism we consider a For loop example with Python multiprocessing module mpi4py for calculating the sum of numbers from a=1 to b = 1000000.  In MPI for Python, MPI.Comm is the base class of communicators.
...
In PTB we have the possibility to use Linux-cluster with the login- head node- head node e00073 and 24 high memory nodes with feauture E52690v4. In order to have an access to the system a registration is requiered by using this Opens external link in new windowform.
